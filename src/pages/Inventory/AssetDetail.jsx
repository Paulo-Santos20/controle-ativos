...
const AssetDetail = () => {
  ...
  // --- PROTEÇÃO DE ERRO ---
  if (errorAsset) {
    return <p className={styles.errorText}>Erro ao carregar: {errorAsset.message}</p>;
  }
  ...
  if (!asset) {
    return (
      <div className={styles.errorState}>
        ...
        <p>O item pode ter sido excluído ou o ID está incorreto.</p>
      ...
      <header className={styles.header}>
        ...
        <div className={styles.actions}>
          {permissions?.ativos?.read && (
            <button className={styles.iconButton} onClick={() => handleOpenModal('qr')} title="Gerar QR Code">
              <QrCode size={20} />
            </button>
          )}
